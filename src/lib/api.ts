...
async function request<T>(path: string, options: RequestOptions = {}): Promise<T> {
  ...
  const requestToken = auth ? (options.token === undefined ? getToken() : options.token) : null;
  if (auth && options.token !== undefined && options.token !== getToken()) {
    throw new ApiError('Account changed. Sign in to the original account to sync its pending changes.', 401);
  }
...
// refreshAccount().
export function getAccount(token?: string | null): Promise<AccountUpdateResponse> {
  return request('/api/account', { method: 'GET', token });
}
...
export function fetchData(token?: string | null): Promise<DataResponse> {
  return request('/api/data', { token });
}

export function putData(blob: unknown, expectedUpdatedAt: string, token?: string | null): Promise<{ updatedAt: string }> {
  return request('/api/data', { method: 'PUT', body: { blob, expectedUpdatedAt }, token });
}
...
export async function uploadPhoto(blob: Blob): Promise<{ url: string; key: string }> {
  ...
  try {
    let response: Response;
    try {
      response = await fetch(`${API_BASE_URL}/api/photos`, { method: 'POST', signal: controller.signal, headers, body: blob });
    } catch {
      throw new ApiError('Could not reach the server. Check your connection.', 0);
    }
